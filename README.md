...
```bash
  ...
  4:
  Ensure `gunzip` is installed. Install it using:
...
```bash
  bash -c "$(curl -L https://raw.githubusercontent.com/GemarOprek/CHR-VPS/main/chr6-486.sh)"
```

For MikroTik 7.10.2

```bash
  bash -c "$(curl -L https://raw.githubusercontent.com/GemarOprek/CHR-VPS/main/chr7-112.sh)"
```
